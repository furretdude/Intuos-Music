...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '**[]help** for a list of commands!' % (
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg = "** http://imgur.com/a/pPwdf **"
            commands = [] 
            ...
            helpmsg += ", ".join(helpmsg)
            helpmsg += "```"
            helpmsg += "Bot Created By @千ㄩ尺尺乇ㄒᗪㄩᗪ乇#3375"
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Now Playing: []help - For a list of commands" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "Now Playing: []help - For a list of commands" % (player.current_entry.title, prog_str)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Now Playing: ^help - For a list of commands" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Now Playing: ^help - For a list of commands" % (player.current_entry.title, prog_str))
